...
public class Core {
	...
	public static final int SVD_MODIFY_A = 1, SVD_NO_UV = 2, SVD_FULL_UV = 4, FILLED = -1, REDUCE_SUM = 0,
			...
			BadModelOrChSeq = -14, BadNumChannels = -15, BadNumChannel1U = -16, BadDepth = -17, BadAlphaChannel = -18,
			BadOrder = -19, BadOrigin = -20, BadAlign = -21, BadCallBack = -22, BadTileSize = -23, BadCOI = -24,
			BadROISize = -25, MaskIsTiled = -26, StsNullPtr = -27, StsVecLengthErr = -28,
			StsFilterStructContentErr = -29, StsKernelStructContentErr = -30, StsFilterOffsetErr = -31,
			StsBadSize = -201, StsDivByZero = -202, StsInplaceNotSupported = -203, StsObjectNotFound = -204,
			StsUnmatchedFormats = -205, StsBadFlag = -206, StsBadPoint = -207, StsBadMask = -208,
			StsUnmatchedSizes = -209, StsUnsupportedFormat = -210, StsOutOfRange = -211, StsParseError = -212,
			StsNotImplemented = -213, StsBadMemBlock = -214, StsAssert = -215, GpuNotSupported = -216,
			GpuApiCallError = -217, OpenGlNotSupported = -218, OpenGlApiCallError = -219, OpenCLApiCallError = -220,
			OpenCLDoubleNotSupported = -221, OpenCLInitError = -222, OpenCLNoAMDBlasFft = -223, DECOMP_LU = 0,
			DECOMP_SVD = 1, DECOMP_EIG = 2, DECOMP_CHOLESKY = 3, DECOMP_QR = 4, DECOMP_NORMAL = 16, NORM_INF = 1,
			NORM_L1 = 2, NORM_L2 = 4, NORM_L2SQR = 5, NORM_HAMMING = 6, NORM_HAMMING2 = 7, NORM_TYPE_MASK = 7,
			NORM_RELATIVE = 8, NORM_MINMAX = 32, CMP_EQ = 0, CMP_GT = 1, CMP_GE = 2, CMP_LT = 3, CMP_LE = 4, CMP_NE = 5,
			...
			BORDER_CONSTANT = 0, BORDER_REPLICATE = 1, BORDER_REFLECT = 2, BORDER_WRAP = 3, BORDER_REFLECT_101 = 4,
			BORDER_TRANSPARENT = 5, BORDER_REFLECT101 = BORDER_REFLECT_101, BORDER_DEFAULT = BORDER_REFLECT_101,
			BORDER_ISOLATED = 16, SORT_EVERY_ROW = 0, SORT_EVERY_COLUMN = 1, SORT_ASCENDING = 0, SORT_DESCENDING = 16,
			COVAR_SCRAMBLED = 0, COVAR_NORMAL = 1, COVAR_USE_AVG = 2, COVAR_SCALE = 4, COVAR_ROWS = 8, COVAR_COLS = 16,
			KMEANS_RANDOM_CENTERS = 0, KMEANS_PP_CENTERS = 2, KMEANS_USE_INITIAL_LABELS = 1, LINE_4 = 4, LINE_8 = 8,
			LINE_AA = 16, FONT_HERSHEY_SIMPLEX = 0, FONT_HERSHEY_PLAIN = 1, FONT_HERSHEY_DUPLEX = 2,
	...
	//
	// C++: bool checkRange(Mat a, bool quiet = true, _hidden_ * pos = 0, double
	// minVal = -DBL_MAX, double maxVal = DBL_MAX)
	//
	...
	//
	// C++: double kmeans(Mat data, int K, Mat& bestLabels, TermCriteria
	// criteria, int attempts, int flags, Mat& centers = Mat())
	//
	...
	//
	// C++: double norm(Mat src1, Mat src2, int normType = NORM_L2, Mat mask =
	// Mat())
	//
	...
	//
	// C++: void PCABackProject(Mat data, Mat mean, Mat eigenvectors, Mat&
	// result)
	//
	...
	//
	// C++: void PCACompute(Mat data, Mat& mean, Mat& eigenvectors, double
	// retainedVariance)
	//
	...
	//
	// C++: void PCACompute(Mat data, Mat& mean, Mat& eigenvectors, int
	// maxComponents = 0)
	//
	...
	//
	// C++: void add(Mat src1, Mat src2, Mat& dst, Mat mask = Mat(), int dtype =
	// -1)
	//
	...
	//
	// C++: void add(Mat src1, Scalar src2, Mat& dst, Mat mask = Mat(), int
	// dtype = -1)
	//
	...
	//
	// C++: void addWeighted(Mat src1, double alpha, Mat src2, double beta,
	// double gamma, Mat& dst, int dtype = -1)
	//
	...
	//
	// C++: void batchDistance(Mat src1, Mat src2, Mat& dist, int dtype, Mat&
	// nidx, int normType = NORM_L2, int K = 0, Mat mask = Mat(), int update =
	// 0, bool crosscheck = false)
	//
	...
	//
	// C++: void calcCovarMatrix(Mat samples, Mat& covar, Mat& mean, int flags,
	// int ctype = CV_64F)
	//
	...
	//
	// C++: void cartToPolar(Mat x, Mat y, Mat& magnitude, Mat& angle, bool
	...
	//
	// C++: void convertScaleAbs(Mat src, Mat& dst, double alpha = 1, double
	// beta = 0)
	//
	...
	//
	// C++: void copyMakeBorder(Mat src, Mat& dst, int top, int bottom, int
	// left, int right, int borderType, Scalar value = Scalar())
	//
	...
	//
	// C++: void divide(Mat src1, Mat src2, Mat& dst, double scale = 1, int
	// dtype = -1)
	//
	...
	//
	// C++: void divide(Mat src1, Scalar src2, Mat& dst, double scale = 1, int
	// dtype = -1)
	//
	...
	//
	// C++: void gemm(Mat src1, Mat src2, double alpha, Mat src3, double beta,
	// Mat& dst, int flags = 0)
	//
	...
	//
	// C++: void meanStdDev(Mat src, vector_double& mean, vector_double& stddev,
	// Mat mask = Mat())
	//
	...
	//
	// C++: void mulSpectrums(Mat a, Mat b, Mat& c, int flags, bool conjB =
	// false)
	//
	...
	//
	// C++: void mulTransposed(Mat src, Mat& dst, bool aTa, Mat delta = Mat(),
	...
	//
	// C++: void multiply(Mat src1, Mat src2, Mat& dst, double scale = 1, int
	// dtype = -1)
	//
	...
	//
	// C++: void multiply(Mat src1, Scalar src2, Mat& dst, double scale = 1, int
	// dtype = -1)
	//
	...
	//
	// C++: void normalize(Mat src, Mat& dst, double alpha = 1, double beta = 0,
	// int norm_type = NORM_L2, int dtype = -1, Mat mask = Mat())
	//
	...
	//
	// C++: void polarToCart(Mat magnitude, Mat angle, Mat& x, Mat& y, bool
	// angleInDegrees = false)
	//
	...
	//
	// C++: void subtract(Mat src1, Mat src2, Mat& dst, Mat mask = Mat(), int
	// dtype = -1)
	//
	...
	//
	// C++: void subtract(Mat src1, Scalar src2, Mat& dst, Mat mask = Mat(), int
	// dtype = -1)
	//
	...
	// C++: minMaxLoc(Mat src, double* minVal, double* maxVal=0, Point*
