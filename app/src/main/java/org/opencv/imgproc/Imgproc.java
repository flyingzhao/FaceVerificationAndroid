...
public class Imgproc {
	...
	public static final int LINE_AA = 16, LINE_8 = 8, LINE_4 = 4, CV_BLUR_NO_SCALE = 0, CV_BLUR = 1, CV_GAUSSIAN = 2,
			CV_MEDIAN = 3, CV_BILATERAL = 4, CV_GAUSSIAN_5x5 = 7, CV_SCHARR = -1, CV_MAX_SOBEL_KSIZE = 7,
			CV_RGBA2mRGBA = 125, CV_mRGBA2RGBA = 126, CV_WARP_FILL_OUTLIERS = 8, CV_WARP_INVERSE_MAP = 16,
			CV_SHAPE_RECT = 0, CV_SHAPE_CROSS = 1, CV_SHAPE_ELLIPSE = 2, CV_SHAPE_CUSTOM = 100, CV_CHAIN_CODE = 0,
			...
			CV_HOUGH_PROBABILISTIC = 1, CV_HOUGH_MULTI_SCALE = 2, CV_HOUGH_GRADIENT = 3, MORPH_ERODE = 0,
			MORPH_DILATE = 1, MORPH_OPEN = 2, MORPH_CLOSE = 3, MORPH_GRADIENT = 4, MORPH_TOPHAT = 5, MORPH_BLACKHAT = 6,
			MORPH_HITMISS = 7, MORPH_RECT = 0, MORPH_CROSS = 1, MORPH_ELLIPSE = 2, INTER_NEAREST = 0, INTER_LINEAR = 1,
			...
			THRESH_TOZERO_INV = 4, THRESH_MASK = 7, THRESH_OTSU = 8, THRESH_TRIANGLE = 16, ADAPTIVE_THRESH_MEAN_C = 0,
			ADAPTIVE_THRESH_GAUSSIAN_C = 1, PROJ_SPHERICAL_ORTHO = 0, PROJ_SPHERICAL_EQRECT = 1, GC_BGD = 0, GC_FGD = 1,
			GC_PR_BGD = 2, GC_PR_FGD = 3, GC_INIT_WITH_RECT = 0, GC_INIT_WITH_MASK = 1, GC_EVAL = 2,
			DIST_LABEL_CCOMP = 0, DIST_LABEL_PIXEL = 1, FLOODFILL_FIXED_RANGE = 1 << 16, FLOODFILL_MASK_ONLY = 1 << 17,
			CC_STAT_LEFT = 0, CC_STAT_TOP = 1, CC_STAT_WIDTH = 2, CC_STAT_HEIGHT = 3, CC_STAT_AREA = 4, CC_STAT_MAX = 5,
			RETR_EXTERNAL = 0, RETR_LIST = 1, RETR_CCOMP = 2, RETR_TREE = 3, RETR_FLOODFILL = 4, CHAIN_APPROX_NONE = 1,
			CHAIN_APPROX_SIMPLE = 2, CHAIN_APPROX_TC89_L1 = 3, CHAIN_APPROX_TC89_KCOS = 4, HOUGH_STANDARD = 0,
			HOUGH_PROBABILISTIC = 1, HOUGH_MULTI_SCALE = 2, HOUGH_GRADIENT = 3, LSD_REFINE_NONE = 0, LSD_REFINE_STD = 1,
			LSD_REFINE_ADV = 2, HISTCMP_CORREL = 0, HISTCMP_CHISQR = 1, HISTCMP_INTERSECT = 2,
			...
			COLOR_BayerGR2RGB_EA = COLOR_BayerGB2BGR_EA, COLOR_COLORCVT_MAX = 139, INTERSECT_NONE = 0,
	...
	//
	// C++: Mat getDefaultNewCameraMatrix(Mat cameraMatrix, Size imgsize =
	// Size(), bool centerPrincipalPoint = false)
	//
	...
	//
	// C++: Mat getGaborKernel(Size ksize, double sigma, double theta, double
	// lambd, double gamma, double psi = CV_PI*0.5, int ktype = CV_64F)
	//
	...
	//
	// C++: Mat getStructuringElement(int shape, Size ksize, Point anchor =
	// Point(-1,-1))
	//
	...
	//
	// C++: Ptr_LineSegmentDetector createLineSegmentDetector(int _refine =
	// LSD_REFINE_STD, double _scale = 0.8, double _sigma_scale = 0.6, double
	// _quant = 2.0, double _ang_th = 22.5, double _log_eps = 0, double
	// _density_th = 0.7, int _n_bins = 1024)
	//
	...
	public static LineSegmentDetector createLineSegmentDetector(int _refine, double _scale, double _sigma_scale,
		...
		LineSegmentDetector retVal = new LineSegmentDetector(createLineSegmentDetector_0(_refine, _scale, _sigma_scale,
				_quant, _ang_th, _log_eps, _density_th, _n_bins));
	...
	//
	// C++: double matchShapes(Mat contour1, Mat contour2, int method, double
	// parameter)
	//
	...
	//
	// C++: double pointPolygonTest(vector_Point2f contour, Point2f pt, bool
	// measureDist)
	//
	...
	//
	// C++: double threshold(Mat src, Mat& dst, double thresh, double maxval,
	// int type)
	//
	...
	//
	// C++: float initWideAngleProjMap(Mat cameraMatrix, Mat distCoeffs, Size
	// imageSize, int destImageWidth, int m1type, Mat& map1, Mat& map2, int
	// projType = PROJ_SPHERICAL_EQRECT, double alpha = 0)
	//

	// javadoc: initWideAngleProjMap(cameraMatrix, distCoeffs, imageSize,
	...
	// javadoc: initWideAngleProjMap(cameraMatrix, distCoeffs, imageSize,
	// destImageWidth, m1type, map1, map2)
	public static float initWideAngleProjMap(Mat cameraMatrix, Mat distCoeffs, Size imageSize, int destImageWidth,
	...
	//
	// C++: float intersectConvexConvex(Mat _p1, Mat _p2, Mat& _p12, bool
	// handleNested = true)
	//
	...
	//
	// C++: int connectedComponents(Mat image, Mat& labels, int connectivity =
	// 8, int ltype = CV_32S)
	//
	...
	//
	// C++: int connectedComponentsWithStats(Mat image, Mat& labels, Mat& stats,
	// Mat& centroids, int connectivity = 8, int ltype = CV_32S)
	//
	...
	//
	// C++: int floodFill(Mat& image, Mat& mask, Point seedPoint, Scalar newVal,
	...
	//
	// C++: int rotatedRectangleIntersection(RotatedRect rect1, RotatedRect
	// rect2, Mat& intersectingRegion)
	//
	...
	public static int rotatedRectangleIntersection(RotatedRect rect1, RotatedRect rect2, Mat intersectingRegion) {
		...
		int retVal = rotatedRectangleIntersection_0(rect1.center.x, rect1.center.y, rect1.size.width, rect1.size.height,
	...
	//
	// C++: void Canny(Mat image, Mat& edges, double threshold1, double
	...
	//
	// C++: void GaussianBlur(Mat src, Mat& dst, Size ksize, double sigmaX,
	...
	//
	// C++: void HoughCircles(Mat image, Mat& circles, int method, double dp,
	// double minDist, double param1 = 100, double param2 = 100, int minRadius =
	// 0, int maxRadius = 0)
	//
	...
	//
	// C++: void HoughLines(Mat image, Mat& lines, double rho, double theta, int
	// threshold, double srn = 0, double stn = 0, double min_theta = 0, double
	// max_theta = CV_PI)
	//
	...
	//
	// C++: void HoughLinesP(Mat image, Mat& lines, double rho, double theta,
	...
	//
	// C++: void Laplacian(Mat src, Mat& dst, int ddepth, int ksize = 1, double
	...
	//
	// C++: void Scharr(Mat src, Mat& dst, int ddepth, int dx, int dy, double
	// scale = 1, double delta = 0, int borderType = BORDER_DEFAULT)
	//
	...
	//
	// C++: void Sobel(Mat src, Mat& dst, int ddepth, int dx, int dy, int ksize
	// = 3, double scale = 1, double delta = 0, int borderType = BORDER_DEFAULT)
	//
	...
	//
	// C++: void accumulateProduct(Mat src1, Mat src2, Mat& dst, Mat mask =
	// Mat())
	//
	...
	//
	// C++: void accumulateWeighted(Mat src, Mat& dst, double alpha, Mat mask =
	// Mat())
	//
	...
	//
	// C++: void adaptiveThreshold(Mat src, Mat& dst, double maxValue, int
	// adaptiveMethod, int thresholdType, int blockSize, double C)
	//

	// javadoc: adaptiveThreshold(src, dst, maxValue, adaptiveMethod,
	...
	//
	// C++: void approxPolyDP(vector_Point2f curve, vector_Point2f& approxCurve,
	...
	//
	// C++: void arrowedLine(Mat& img, Point pt1, Point pt2, Scalar color, int
	// thickness = 1, int line_type = 8, int shift = 0, double tipLength = 0.1)
	//
	...
	//
	// C++: void bilateralFilter(Mat src, Mat& dst, int d, double sigmaColor,
	...
	//
	// C++: void blur(Mat src, Mat& dst, Size ksize, Point anchor =
	...
	//
	// C++: void boxFilter(Mat src, Mat& dst, int ddepth, Size ksize, Point
	// anchor = Point(-1,-1), bool normalize = true, int borderType =
	// BORDER_DEFAULT)
	//
	...
	//
	// C++: void calcBackProject(vector_Mat images, vector_int channels, Mat
	// hist, Mat& dst, vector_float ranges, double scale)
	//
	...
	//
	// C++: void calcHist(vector_Mat images, vector_int channels, Mat mask, Mat&
	// hist, vector_int histSize, vector_float ranges, bool accumulate = false)
	//
	...
	//
	// C++: void circle(Mat& img, Point center, int radius, Scalar color, int
	...
	//
	// C++: void convertMaps(Mat map1, Mat map2, Mat& dstmap1, Mat& dstmap2, int
	...
	//
	// C++: void convexHull(vector_Point points, vector_int& hull, bool
	// clockwise = false, _hidden_ returnPoints = true)
	//
	...
	//
	// C++: void convexityDefects(vector_Point contour, vector_int convexhull,
	// vector_Vec4i& convexityDefects)
	//
	...
	//
	// C++: void cornerEigenValsAndVecs(Mat src, Mat& dst, int blockSize, int
	// ksize, int borderType = BORDER_DEFAULT)
	//
	...
	//
	// C++: void cornerHarris(Mat src, Mat& dst, int blockSize, int ksize,
	...
	//
	// C++: void cornerMinEigenVal(Mat src, Mat& dst, int blockSize, int ksize =
	...
	public static void cornerSubPix(Mat image, MatOfPoint2f corners, Size winSize, Size zeroZone,
			TermCriteria criteria) {
		Mat corners_mat = corners;
		cornerSubPix_0(image.nativeObj, corners_mat.nativeObj, winSize.width, winSize.height, zeroZone.width,
	...
	//
	// C++: void dilate(Mat src, Mat& dst, Mat kernel, Point anchor =
	// Point(-1,-1), int iterations = 1, int borderType = BORDER_CONSTANT,
	...
	//
	// C++: void distanceTransform(Mat src, Mat& dst, Mat& labels, int
	// distanceType, int maskSize, int labelType = DIST_LABEL_CCOMP)
	//
	...
	//
	// C++: void distanceTransform(Mat src, Mat& dst, int distanceType, int
	// maskSize, int dstType = CV_32F)
	//
	...
	//
	// C++: void drawContours(Mat& image, vector_vector_Point contours, int
	// contourIdx, Scalar color, int thickness = 1, int lineType = LINE_8, Mat
	// hierarchy = Mat(), int maxLevel = INT_MAX, Point offset = Point())
	//
	...
	//
	// C++: void drawMarker(Mat& img, Point position, Scalar color, int
	// markerType = MARKER_CROSS, int markerSize = 20, int thickness = 1, int
	// line_type = 8)
	//
	...
	//
	// C++: void ellipse(Mat& img, Point center, Size axes, double angle, double
	// startAngle, double endAngle, Scalar color, int thickness = 1, int
	...
	//
	// C++: void ellipse(Mat& img, RotatedRect box, Scalar color, int thickness
	// = 1, int lineType = LINE_8)
	//
	...
	public static void ellipse(Mat img, RotatedRect box, Scalar color, int thickness, int lineType) {
		...
		ellipse_3(img.nativeObj, box.center.x, box.center.y, box.size.width, box.size.height, box.angle, color.val[0],
	...
	//
	// C++: void ellipse2Poly(Point center, Size axes, int angle, int arcStart,
	...
	//
	// C++: void erode(Mat src, Mat& dst, Mat kernel, Point anchor =
	// Point(-1,-1), int iterations = 1, int borderType = BORDER_CONSTANT,
	// Scalar borderValue = morphologyDefaultBorderValue())
	//
	...
	//
	// C++: void fillConvexPoly(Mat& img, vector_Point points, Scalar color, int
	// lineType = LINE_8, int shift = 0)
	//
	...
	//
	// C++: void fillPoly(Mat& img, vector_vector_Point pts, Scalar color, int
	// lineType = LINE_8, int shift = 0, Point offset = Point())
	//
	...
	//
	// C++: void filter2D(Mat src, Mat& dst, int ddepth, Mat kernel, Point
	...
	//
	// C++: void findContours(Mat& image, vector_vector_Point& contours, Mat&
	...
	//
	// C++: void fitLine(Mat points, Mat& line, int distType, double param,
	...
	//
	// C++: void getDerivKernels(Mat& kx, Mat& ky, int dx, int dy, int ksize,
	...
	//
	// C++: void getRectSubPix(Mat image, Size patchSize, Point2f center, Mat&
	// patch, int patchType = -1)
	//
	...
	//
	// C++: void goodFeaturesToTrack(Mat image, vector_Point& corners, int
	// maxCorners, double qualityLevel, double minDistance, Mat mask = Mat(),
	// int blockSize = 3, bool useHarrisDetector = false, double k = 0.04)
	//

	// javadoc: goodFeaturesToTrack(image, corners, maxCorners, qualityLevel,
	...
	//
	// C++: void grabCut(Mat img, Mat& mask, Rect rect, Mat& bgdModel, Mat&
	// fgdModel, int iterCount, int mode = GC_EVAL)
	//
	...
	//
	// C++: void initUndistortRectifyMap(Mat cameraMatrix, Mat distCoeffs, Mat
	// R, Mat newCameraMatrix, Size size, int m1type, Mat& map1, Mat& map2)
	//

	// javadoc: initUndistortRectifyMap(cameraMatrix, distCoeffs, R,
	// newCameraMatrix, size, m1type, map1, map2)
	public static void initUndistortRectifyMap(Mat cameraMatrix, Mat distCoeffs, Mat R, Mat newCameraMatrix, Size size,
			int m1type, Mat map1, Mat map2) {

		initUndistortRectifyMap_0(cameraMatrix.nativeObj, distCoeffs.nativeObj, R.nativeObj, newCameraMatrix.nativeObj,
	...
	//
	// C++: void integral(Mat src, Mat& sum, Mat& sqsum, Mat& tilted, int sdepth
	// = -1, int sqdepth = -1)
	//
	...
	//
	// C++: void integral(Mat src, Mat& sum, Mat& sqsum, int sdepth = -1, int
	// sqdepth = -1)
	//
	...
	//
	// C++: void line(Mat& img, Point pt1, Point pt2, Scalar color, int
	// thickness = 1, int lineType = LINE_8, int shift = 0)
	//
	...
	//
	// C++: void linearPolar(Mat src, Mat& dst, Point2f center, double
	// maxRadius, int flags)
	//
	...
	//
	// C++: void logPolar(Mat src, Mat& dst, Point2f center, double M, int
	// flags)
	//
	...
	//
	// C++: void minEnclosingCircle(vector_Point2f points, Point2f& center,
	// float& radius)
	//
	...
	//
	// C++: void morphologyEx(Mat src, Mat& dst, int op, Mat kernel, Point
	// anchor = Point(-1,-1), int iterations = 1, int borderType =
	// BORDER_CONSTANT, Scalar borderValue = morphologyDefaultBorderValue())
	//
	...
	//
	// C++: void polylines(Mat& img, vector_vector_Point pts, bool isClosed,
	...
	//
	// C++: void preCornerDetect(Mat src, Mat& dst, int ksize, int borderType =
	// BORDER_DEFAULT)
	//
	...
	//
	// C++: void putText(Mat& img, String text, Point org, int fontFace, double
	// fontScale, Scalar color, int thickness = 1, int lineType = LINE_8, bool
	...
	//
	// C++: void pyrDown(Mat src, Mat& dst, Size dstsize = Size(), int
	...
	//
	// C++: void pyrMeanShiftFiltering(Mat src, Mat& dst, double sp, double sr,
	// int maxLevel = 1, TermCriteria termcrit =
	...
	//
	// C++: void pyrUp(Mat src, Mat& dst, Size dstsize = Size(), int borderType
	// = BORDER_DEFAULT)
	//
	...
	//
	// C++: void rectangle(Mat& img, Point pt1, Point pt2, Scalar color, int
	// thickness = 1, int lineType = LINE_8, int shift = 0)
	//
	...
	//
	// C++: void remap(Mat src, Mat& dst, Mat map1, Mat map2, int interpolation,
	...
	//
	// C++: void resize(Mat src, Mat& dst, Size dsize, double fx = 0, double fy
	...
	//
	// C++: void sepFilter2D(Mat src, Mat& dst, int ddepth, Mat kernelX, Mat
	// kernelY, Point anchor = Point(-1,-1), double delta = 0, int borderType =
	// BORDER_DEFAULT)
	//
	...
	//
	// C++: void spatialGradient(Mat src, Mat& dx, Mat& dy, int ksize = 3, int
	// borderType = BORDER_DEFAULT)
	//
	...
	//
	// C++: void sqrBoxFilter(Mat _src, Mat& _dst, int ddepth, Size ksize, Point
	// anchor = Point(-1, -1), bool normalize = true, int borderType =
	// BORDER_DEFAULT)
	//
	...
	public static void sqrBoxFilter(Mat _src, Mat _dst, int ddepth, Size ksize, Point anchor, boolean normalize,
		...
		sqrBoxFilter_0(_src.nativeObj, _dst.nativeObj, ddepth, ksize.width, ksize.height, anchor.x, anchor.y, normalize,
				borderType);
	...
	//
	// C++: void undistort(Mat src, Mat& dst, Mat cameraMatrix, Mat distCoeffs,
	...
	//
	// C++: void undistortPoints(vector_Point2f src, vector_Point2f& dst, Mat
	// cameraMatrix, Mat distCoeffs, Mat R = Mat(), Mat P = Mat())
	//
	...
	public static void undistortPoints(MatOfPoint2f src, MatOfPoint2f dst, Mat cameraMatrix, Mat distCoeffs, Mat R,
		...
		Mat src_mat = src;
		Mat dst_mat = dst;
		undistortPoints_0(src_mat.nativeObj, dst_mat.nativeObj, cameraMatrix.nativeObj, distCoeffs.nativeObj,
	...
	//
	// C++: void warpAffine(Mat src, Mat& dst, Mat M, Size dsize, int flags =
	// INTER_LINEAR, int borderMode = BORDER_CONSTANT, Scalar borderValue =
	// Scalar())
	//
	...
	//
	// C++: void warpPerspective(Mat src, Mat& dst, Mat M, Size dsize, int flags
	// = INTER_LINEAR, int borderMode = BORDER_CONSTANT, Scalar borderValue =
	// Scalar())
	//
	...
	private static native void Canny_1(long image_nativeObj, long edges_nativeObj, double threshold1,
			double threshold2);
	...
	// C++: void cornerSubPix(Mat image, vector_Point2f& corners, Size winSize,
	// Size zeroZone, TermCriteria criteria)
	private static native void cornerSubPix_0(long image_nativeObj, long corners_mat_nativeObj, double winSize_width,
	...
	// anchor = Point(-1, -1), bool normalize = true, int borderType =
	// BORDER_DEFAULT)
	private static native void sqrBoxFilter_0(long _src_nativeObj, long _dst_nativeObj, int ddepth, double ksize_width,
			double ksize_height, double anchor_x, double anchor_y, boolean normalize, int borderType);

	private static native void sqrBoxFilter_1(long _src_nativeObj, long _dst_nativeObj, int ddepth, double ksize_width,
			double ksize_height, double anchor_x, double anchor_y, boolean normalize);

	private static native void sqrBoxFilter_2(long _src_nativeObj, long _dst_nativeObj, int ddepth, double ksize_width,
			double ksize_height);
