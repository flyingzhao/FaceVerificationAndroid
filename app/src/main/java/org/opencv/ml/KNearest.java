...
public class KNearest extends StatModel {
	...
	//
	// C++: float findNearest(Mat samples, int k, Mat& results, Mat&
	// neighborResponses = Mat(), Mat& dist = Mat())
	//
	...
	public float findNearest(Mat samples, int k, Mat results, Mat neighborResponses, Mat dist) {
		...
		float retVal = findNearest_0(nativeObj, samples.nativeObj, k, results.nativeObj, neighborResponses.nativeObj,
				dist.nativeObj);
