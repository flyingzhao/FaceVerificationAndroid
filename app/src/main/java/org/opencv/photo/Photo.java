...
public class Photo {
	...
	private static final int CV_INPAINT_NS = 0, CV_INPAINT_TELEA = 1;

	public static final int INPAINT_NS = 0, INPAINT_TELEA = 1, NORMAL_CLONE = 1, MIXED_CLONE = 2,
			MONOCHROME_TRANSFER = 3, RECURS_FILTER = 1, NORMCONV_FILTER = 2, LDR_SIZE = 256;

	//
	// C++: Ptr_AlignMTB createAlignMTB(int max_bits = 6, int exclude_range = 4,
	// bool cut = true)
	//
	...
	//
	// C++: Ptr_CalibrateDebevec createCalibrateDebevec(int samples = 70, float
	// lambda = 10.0f, bool random = false)
	//
	...
	//
	// C++: Ptr_CalibrateRobertson createCalibrateRobertson(int max_iter = 30,
	...
	//
	// C++: Ptr_MergeMertens createMergeMertens(float contrast_weight = 1.0f,
	// float saturation_weight = 1.0f, float exposure_weight = 0.0f)
	//
	...
	public static MergeMertens createMergeMertens(float contrast_weight, float saturation_weight,
		...
		MergeMertens retVal = new MergeMertens(
				createMergeMertens_0(contrast_weight, saturation_weight, exposure_weight));
	...
	//
	// C++: Ptr_TonemapDrago createTonemapDrago(float gamma = 1.0f, float
	// saturation = 1.0f, float bias = 0.85f)
	//
	...
	//
	// C++: Ptr_TonemapDurand createTonemapDurand(float gamma = 1.0f, float
	// contrast = 4.0f, float saturation = 1.0f, float sigma_space = 2.0f, float
	// sigma_color = 2.0f)
	//
	...
	public static TonemapDurand createTonemapDurand(float gamma, float contrast, float saturation, float sigma_space,
		...
		TonemapDurand retVal = new TonemapDurand(
				createTonemapDurand_0(gamma, contrast, saturation, sigma_space, sigma_color));
	...
	//
	// C++: Ptr_TonemapMantiuk createTonemapMantiuk(float gamma = 1.0f, float
	// scale = 0.7f, float saturation = 1.0f)
	//
	...
	//
	// C++: Ptr_TonemapReinhard createTonemapReinhard(float gamma = 1.0f, float
	// intensity = 0.0f, float light_adapt = 1.0f, float color_adapt = 0.0f)
	//
	...
	public static TonemapReinhard createTonemapReinhard(float gamma, float intensity, float light_adapt,
		...
		TonemapReinhard retVal = new TonemapReinhard(
				createTonemapReinhard_0(gamma, intensity, light_adapt, color_adapt));
	...
	//
	// C++: void colorChange(Mat src, Mat mask, Mat& dst, float red_mul = 1.0f,
	...
	//
	// C++: void denoise_TVL1(vector_Mat observations, Mat result, double lambda
	// = 1.0, int niters = 30)
	//
	...
	//
	// C++: void edgePreservingFilter(Mat src, Mat& dst, int flags = 1, float
	// sigma_s = 60, float sigma_r = 0.4f)
	//
	...
	//
	// C++: void fastNlMeansDenoising(Mat src, Mat& dst, float h = 3, int
	...
	//
	// C++: void fastNlMeansDenoising(Mat src, Mat& dst, vector_float h, int
	// templateWindowSize = 7, int searchWindowSize = 21, int normType =
	// NORM_L2)
	//
	...
	public static void fastNlMeansDenoising(Mat src, Mat dst, MatOfFloat h, int templateWindowSize,
		...
		Mat h_mat = h;
		fastNlMeansDenoising_2(src.nativeObj, dst.nativeObj, h_mat.nativeObj, templateWindowSize, searchWindowSize,
				normType);
	...
	//
	// C++: void fastNlMeansDenoisingColored(Mat src, Mat& dst, float h = 3,
	// float hColor = 3, int templateWindowSize = 7, int searchWindowSize = 21)
	//
	...
	//
	// C++: void fastNlMeansDenoisingColoredMulti(vector_Mat srcImgs, Mat& dst,
	// int imgToDenoiseIndex, int temporalWindowSize, float h = 3, float hColor
	// = 3, int templateWindowSize = 7, int searchWindowSize = 21)
	//

	// javadoc: fastNlMeansDenoisingColoredMulti(srcImgs, dst,
	// imgToDenoiseIndex, temporalWindowSize, h, hColor, templateWindowSize,
	// searchWindowSize)
	public static void fastNlMeansDenoisingColoredMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
			int temporalWindowSize, float h, float hColor, int templateWindowSize, int searchWindowSize) {
		Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
		fastNlMeansDenoisingColoredMulti_0(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize,
	...
	//
	// C++: void fastNlMeansDenoisingMulti(vector_Mat srcImgs, Mat& dst, int
	// imgToDenoiseIndex, int temporalWindowSize, float h = 3, int
	// templateWindowSize = 7, int searchWindowSize = 21)
	//

	// javadoc: fastNlMeansDenoisingMulti(srcImgs, dst, imgToDenoiseIndex,
	// temporalWindowSize, h, templateWindowSize, searchWindowSize)
	public static void fastNlMeansDenoisingMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
			int temporalWindowSize, float h, int templateWindowSize, int searchWindowSize) {
		Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
		fastNlMeansDenoisingMulti_0(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize, h,
				templateWindowSize, searchWindowSize);
	...
	//
	// C++: void fastNlMeansDenoisingMulti(vector_Mat srcImgs, Mat& dst, int
	// imgToDenoiseIndex, int temporalWindowSize, vector_float h, int
	// templateWindowSize = 7, int searchWindowSize = 21, int normType =
	// NORM_L2)
	//
	...
	public static void fastNlMeansDenoisingMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
			int temporalWindowSize, MatOfFloat h, int templateWindowSize, int searchWindowSize, int normType) {
		Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
		Mat h_mat = h;
		fastNlMeansDenoisingMulti_2(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize,
				h_mat.nativeObj, templateWindowSize, searchWindowSize, normType);

		return;
	}

	// javadoc: fastNlMeansDenoisingMulti(srcImgs, dst, imgToDenoiseIndex,
	// temporalWindowSize, h)
	public static void fastNlMeansDenoisingMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
			int temporalWindowSize, MatOfFloat h) {
		Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
		Mat h_mat = h;
	...
	//
	// C++: void illuminationChange(Mat src, Mat mask, Mat& dst, float alpha =
	// 0.2f, float beta = 0.4f)
	//
	...
	//
	// C++: void inpaint(Mat src, Mat inpaintMask, Mat& dst, double
	...
	//
	// C++: void pencilSketch(Mat src, Mat& dst1, Mat& dst2, float sigma_s = 60,
	...
	//
	// C++: void seamlessClone(Mat src, Mat dst, Mat mask, Point p, Mat& blend,
	// int flags)
	//
	...
	//
	// C++: void stylization(Mat src, Mat& dst, float sigma_s = 60, float
	// sigma_r = 0.45f)
	//
	...
	//
	// C++: void textureFlattening(Mat src, Mat mask, Mat& dst, float
	// low_threshold = 30, float high_threshold = 45, int kernel_size = 3)
	//
	...
	// C++: Ptr_MergeMertens createMergeMertens(float contrast_weight = 1.0f,
	// float saturation_weight = 1.0f, float exposure_weight = 0.0f)
	private static native long createMergeMertens_0(float contrast_weight, float saturation_weight,
			float exposure_weight);
	...
	// contrast = 4.0f, float saturation = 1.0f, float sigma_space = 2.0f, float
	// sigma_color = 2.0f)
	private static native long createTonemapDurand_0(float gamma, float contrast, float saturation, float sigma_space,
			float sigma_color);
	...
	// C++: Ptr_TonemapReinhard createTonemapReinhard(float gamma = 1.0f, float
	// intensity = 0.0f, float light_adapt = 1.0f, float color_adapt = 0.0f)
	private static native long createTonemapReinhard_0(float gamma, float intensity, float light_adapt,
			float color_adapt);
	...
	// C++: void colorChange(Mat src, Mat mask, Mat& dst, float red_mul = 1.0f,
	// float green_mul = 1.0f, float blue_mul = 1.0f)
	private static native void colorChange_0(long src_nativeObj, long mask_nativeObj, long dst_nativeObj, float red_mul,
	...
	// C++: void denoise_TVL1(vector_Mat observations, Mat result, double lambda
	// = 1.0, int niters = 30)
	private static native void denoise_TVL1_0(long observations_mat_nativeObj, long result_nativeObj, double lambda,
			int niters);
	...
	// C++: void edgePreservingFilter(Mat src, Mat& dst, int flags = 1, float
	// sigma_s = 60, float sigma_r = 0.4f)
	private static native void edgePreservingFilter_0(long src_nativeObj, long dst_nativeObj, int flags, float sigma_s,
			float sigma_r);
	...
	// C++: void fastNlMeansDenoising(Mat src, Mat& dst, float h = 3, int
	// templateWindowSize = 7, int searchWindowSize = 21)
	private static native void fastNlMeansDenoising_0(long src_nativeObj, long dst_nativeObj, float h,
	...
	// C++: void fastNlMeansDenoisingColored(Mat src, Mat& dst, float h = 3,
	// float hColor = 3, int templateWindowSize = 7, int searchWindowSize = 21)
	private static native void fastNlMeansDenoisingColored_0(long src_nativeObj, long dst_nativeObj, float h,
	...
	// int imgToDenoiseIndex, int temporalWindowSize, float h = 3, float hColor
	// = 3, int templateWindowSize = 7, int searchWindowSize = 21)
	private static native void fastNlMeansDenoisingColoredMulti_0(long srcImgs_mat_nativeObj, long dst_nativeObj,
			int imgToDenoiseIndex, int temporalWindowSize, float h, float hColor, int templateWindowSize,
			int searchWindowSize);

	private static native void fastNlMeansDenoisingColoredMulti_1(long srcImgs_mat_nativeObj, long dst_nativeObj,
			int imgToDenoiseIndex, int temporalWindowSize);

	// C++: void fastNlMeansDenoisingMulti(vector_Mat srcImgs, Mat& dst, int
	// imgToDenoiseIndex, int temporalWindowSize, float h = 3, int
	// templateWindowSize = 7, int searchWindowSize = 21)
	private static native void fastNlMeansDenoisingMulti_0(long srcImgs_mat_nativeObj, long dst_nativeObj,
			int imgToDenoiseIndex, int temporalWindowSize, float h, int templateWindowSize, int searchWindowSize);

	private static native void fastNlMeansDenoisingMulti_1(long srcImgs_mat_nativeObj, long dst_nativeObj,
			int imgToDenoiseIndex, int temporalWindowSize);
	...
	// templateWindowSize = 7, int searchWindowSize = 21, int normType =
	// NORM_L2)
	private static native void fastNlMeansDenoisingMulti_2(long srcImgs_mat_nativeObj, long dst_nativeObj,
			int imgToDenoiseIndex, int temporalWindowSize, long h_mat_nativeObj, int templateWindowSize,
			int searchWindowSize, int normType);

	private static native void fastNlMeansDenoisingMulti_3(long srcImgs_mat_nativeObj, long dst_nativeObj,
			int imgToDenoiseIndex, int temporalWindowSize, long h_mat_nativeObj);

	// C++: void illuminationChange(Mat src, Mat mask, Mat& dst, float alpha =
	// 0.2f, float beta = 0.4f)
	private static native void illuminationChange_0(long src_nativeObj, long mask_nativeObj, long dst_nativeObj,
	...
	// C++: void inpaint(Mat src, Mat inpaintMask, Mat& dst, double
	// inpaintRadius, int flags)
	private static native void inpaint_0(long src_nativeObj, long inpaintMask_nativeObj, long dst_nativeObj,
			double inpaintRadius, int flags);

	// C++: void pencilSketch(Mat src, Mat& dst1, Mat& dst2, float sigma_s = 60,
	// float sigma_r = 0.07f, float shade_factor = 0.02f)
	private static native void pencilSketch_0(long src_nativeObj, long dst1_nativeObj, long dst2_nativeObj,
	...
	// C++: void seamlessClone(Mat src, Mat dst, Mat mask, Point p, Mat& blend,
	// int flags)
	private static native void seamlessClone_0(long src_nativeObj, long dst_nativeObj, long mask_nativeObj, double p_x,
			double p_y, long blend_nativeObj, int flags);
	...
	// C++: void textureFlattening(Mat src, Mat mask, Mat& dst, float
	// low_threshold = 30, float high_threshold = 45, int kernel_size = 3)
	private static native void textureFlattening_0(long src_nativeObj, long mask_nativeObj, long dst_nativeObj,
